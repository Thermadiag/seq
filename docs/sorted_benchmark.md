...
Additional operations could have been tested, like iteration performances, failed insertion, range insertion, different input distributions... I might do it in the future, but my spare time is currently quite limited. Here are just a few hints based on internal benchmarks that I did not take the time to properly formalize:
-	`std::set` is the slowest in all tested scenarios for all operations. Indeed, its requirements (iterator/reference stability) require an allocation/deallocation for each insertion/deletion, and a lot of cache misses for lookup.
...
-	`seq::radix_set` is not comparison based, and can be slower than other containers with adverserial input distribution, like 'a', 'aa', 'aaa',... However, internal guardrails ensure that its performances remain comparable to `seq::flat_set` with an additional time constant.
