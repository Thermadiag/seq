...
namespace seq
{
	namespace detail
	{
		...
		struct base_list_chunk
		{
			...
			static const std::uint64_t full = (count == 64ULL ? (std::uint64_t )-1 : (shift_left<count>::value - 1ULL));
			// Invalid index value
			...
			int end;
			// First free index
			SEQ_ALWAYS_INLINE unsigned firstFree() const noexcept { return (unsigned)bit_scan_forward_64(~used); }
			// First used index
			SEQ_ALWAYS_INLINE unsigned firstUsed() const noexcept { return bit_scan_forward_64(used); }
			// Number of valid (allocated) elements
			SEQ_ALWAYS_INLINE unsigned size() const noexcept { return popcnt64(used); }
		};
		...
		struct list_chunk : public base_list_chunk<T>
		{
			...
			// Storage for values
			typename std::aligned_storage<count * sizeof(T), alignof(T) >::type storage;
			//T storage[count];
			//Raw buffer access
			SEQ_ALWAYS_INLINE T* buffer() noexcept { return  (T*)(&storage); }
			SEQ_ALWAYS_INLINE const T* buffer() const noexcept { return  (T*)(&storage); }
		...
		// Proxy class used to get the size of list_chunk object
		template< class T>
		struct chunk_proxy : public base_list_chunk<T> {
			typename std::aligned_storage<base_list_chunk<T>::count * sizeof(T), alignof(T) >::type storage;
		};

		// Compute padding size to properly align list_chunk on 64 bytes
		template<class T>
		struct chunk_padding_base {
			static const size_t size = (sizeof(chunk_proxy<T>) % 64ULL) ? (((sizeof(chunk_proxy<T>) / 64ULL) + 1ULL) * 64ULL) - sizeof(chunk_proxy<T>) : 0ULL;
		};
		template<class T, size_t Size = chunk_padding_base<T>::size>
		struct chunk_padding {
			char padding[Size];
		};
		template<class T>
		struct chunk_padding<T, 0> {
		};

		// Actual chunk class, store up to 64 objects
		// Its size is a multiple of 64 bytes
		template< class T>
		struct list_chunk : public base_list_chunk<T> //, chunk_padding<T> TEST: remove
		{
			using base_type = base_list_chunk<T>;
			using base_type::count;
			using base_type::count_bits;
		...
		public:
			chunk_pool_alloc(const Allocator& alloc) noexcept : pool(alloc) {}
			chunk_pool_alloc(size_t elem_count, const Allocator& alloc) :pool(alloc) {
				//resize(elem_count);
			}
			~chunk_pool_alloc() {
			}
			...
			chunk_pool_alloc& operator=(const chunk_pool_alloc&) = delete;
		...
		class chunk_pool_alloc : public Allocator
		{
			...
			bool can_use_aligned_alloc() const noexcept {return std::is_same < Allocator, std::allocator<T> >::value && d_pool < 4;}
			bool pool_allocated() const noexcept { return d_pool > 4; }
		...
		public:
			...
			void resize(std::size_t count) {
				if (pool_allocated() || count > 4) {
					...
					{
						rebind_alloc< pool_type> al = get_allocator();
						d_pool = (std::uintptr_t)al.allocate(1);
			...
			list_chunk<T>* allocate_chunk()
			{
				if (can_use_aligned_alloc()) {
					list_chunk<T>* res = (list_chunk<T>*)aligned_malloc(sizeof(list_chunk<T>), 64);
					if (!res)
						throw std::bad_alloc();
					res->user_flag = 0; 
					d_pool++;
		...
		struct std_alloc
		{
			...
			std_alloc(size_t) noexcept :chunks(0) {}
			...
			// capacity in terms of chunk
			size_t get_capacity() const noexcept { return chunks; }
		...
		{
			typedef chunk_pool_alloc<T, Allocator> type;
		};
		...
		public:
			...
			sequence_const_iterator(const chunk_type* node) noexcept
				:node((chunk_type*)node), pos((pos_type)node->start) {}
			sequence_const_iterator(const chunk_type* node, pos_type pos) noexcept
				:node((chunk_type*)node), pos(pos) {}
			...
			void update_incr_pos()
			{
				if (pos == (pos_type)node->end) {
					node = (chunk_type*)node->next;
					pos = (pos_type)node->start;
				}
				else {
					pos = (pos_type)(bit_scan_forward_64(node->used >> pos) + pos);//ptr - node->buffer();
				}
			}
			SEQ_ALWAYS_INLINE sequence_const_iterator<List>& operator++() noexcept {
				++pos;
				if (SEQ_UNLIKELY(pos == (pos_type)count || !((node->used & (1ULL << pos))))) {
					update_incr_pos();
				}
				//SEQ_ASSERT_DEBUG((pos >= 0 && pos < node->end) || (pos == 0 && node == &data->end), "invalid iterator position");
				return *this;
			}
			SEQ_ALWAYS_INLINE sequence_const_iterator operator++(int) noexcept {
				sequence_const_iterator _Tmp = *this;
				++(*this);
				return _Tmp;
			}
			void update_decr_pos()
			{
				if (pos < (pos_type)node->start) {
					node = (detail::list_chunk<value_type>*)node->prev;
					pos = (pos_type)(node->end - 1);
				}
				else {
					//ptr = node->buffer() + bit_scan_forward_64(node->used >> pos) + pos;
					pos = (pos_type)(bit_scan_reverse_64(node->used & ((1ULL << pos) - 1ULL)));//ptr - node->buffer();
				}
			}
			SEQ_ALWAYS_INLINE sequence_const_iterator<List>& operator--() noexcept {
				--pos;
				if (SEQ_UNLIKELY(pos == (pos_type)-1 || !((node->used & (1ULL << pos)))  /*ptr == node->end*/)) {
					update_decr_pos();
				}
			...
			SEQ_ALWAYS_INLINE sequence_const_iterator& operator+=(difference_type diff) {
				increment(diff);
				return *this;
			}
			SEQ_ALWAYS_INLINE sequence_const_iterator& operator-=(difference_type diff) {
			...
			{
				if (diff > 0) {
					unsigned rem = pos == count - 1 ? 0 : popcnt64(node->used >> (pos + 1ULL));
					if (diff <= rem) {
						while (diff--)
							++(*this);
					}
						...
						diff -= rem + 1;
						(*this).node = (chunk_type*)(*this).node->next;
					...
					if (diff <= rem) {
						...
						diff -= rem + 1;
						(*this).node = (chunk_type*)(*this).node->prev;
			...
			static difference_type distance(const sequence_const_iterator& it1, const sequence_const_iterator& it2)
			{
				if (it1.node == it2.node) {
					// Same node
					if (it1.pos > it2.pos) {
						return popcnt64((it1.node->used & ((1ULL << it1.pos) - 1)) >> it2.pos);
					}
					else {
						return -((difference_type)popcnt64((it1.node->used & ((1ULL << it2.pos) - 1)) >> it1.pos));
					}
				}
				else {
					// Different nodes
					...
					diff += rem + 1;
					start.node = (chunk_type*)start.node->next;
		...
		public:
			...
			sequence_iterator(const typename base_type::chunk_type* node)noexcept
				:sequence_const_iterator<List>(node) {}
			sequence_iterator(const typename base_type::chunk_type* node, typename base_type::pos_type pos) noexcept
				:sequence_const_iterator<List>(node, pos) {}
			...
			sequence_iterator& operator+=(difference_type diff)noexcept {
		...
		template<class List>
		SEQ_ALWAYS_INLINE typename sequence_const_iterator<List>::difference_type operator-(const sequence_const_iterator<List>& a, const sequence_const_iterator<List>& b) {
			return a.distance(a, b);
		}
		template < class List>
		SEQ_ALWAYS_INLINE sequence_const_iterator< List> operator+(const sequence_const_iterator< List>& it, typename sequence_const_iterator< List>::difference_type diff) {
			sequence_const_iterator< List> res = it;
			res += diff;
			return res;
		}
		template < class List>
		SEQ_ALWAYS_INLINE sequence_iterator< List> operator+(const sequence_iterator< List>& it, typename sequence_iterator< List>::difference_type diff) {
		...
		struct sequence_ra_iterator
		{
			...
			sequence_ra_iterator(const Data* d, const chunk_type* node) noexcept
				:data(const_cast<Data*>(d)), node(const_cast<chunk_type*>(node)), abs_pos(0), pos((pos_type)node->start) {}
			sequence_ra_iterator(const Data* d, const chunk_type* node, pos_type pos, difference_type abs_pos) noexcept
				:data(const_cast<Data*>(d)), node(const_cast<chunk_type*>(node)), abs_pos(abs_pos), pos(pos) {}


			SEQ_ALWAYS_INLINE std::size_t absolutePos() const noexcept {
				return abs_pos;
			}
			SEQ_ALWAYS_INLINE void setAbsolutePos(std::size_t abs_pos) noexcept {
				if (SEQ_UNLIKELY(abs_pos == data->size)) {
					...
					size_t front_size = (size_t)(data->chunks.front()->end - data->chunks.front()->start);
					size_t bucket = (abs_pos + (chunk_type::count - front_size)) >> chunk_type::count_bits;
					node = data->chunks[bucket];
					pos = node->start + ((abs_pos - (abs_pos < front_size ? 0 : front_size)) & (chunk_type::count - 1));
			...
			SEQ_ALWAYS_INLINE sequence_ra_iterator<List>& operator++() noexcept {
				++pos;
				++abs_pos;
				if (/*pos == (pos_type)count || /!((node->used & (1ULL << pos)))*/pos >= node->end) {
					update_incr();
				}
				//SEQ_ASSERT_DEBUG((pos >= 0 && pos < node->end) || (pos == 0 && node == &data->end), "invalid iterator position");
				return *this;
			}
			SEQ_ALWAYS_INLINE sequence_ra_iterator operator++(int) noexcept {
			...
			void update_decr()
			{
				if (pos < (pos_type)node->start) {
					...
					//ptr = node->buffer() + bit_scan_forward_64(node->used >> pos) + pos;
					pos = (pos_type)(bit_scan_reverse_64(node->used & ((1ULL << pos) - 1)));//ptr - node->buffer();
				}
			}
			SEQ_ALWAYS_INLINE sequence_ra_iterator<List>& operator--() noexcept {
				--pos;
			...
			SEQ_ALWAYS_INLINE sequence_ra_iterator& operator+=(difference_type diff) {
			...
			SEQ_ALWAYS_INLINE bool operator<=(const sequence_ra_iterator& other) const noexcept { return abs_pos <= other.abs_pos; }
			SEQ_ALWAYS_INLINE bool operator>=(const sequence_ra_iterator& other) const noexcept { return abs_pos >= other.abs_pos; }
			SEQ_ALWAYS_INLINE sequence_ra_iterator operator+(difference_type diff) const noexcept {
			...
			SEQ_ALWAYS_INLINE difference_type operator-(const sequence_ra_iterator& other) const noexcept { return abs_pos - other.abs_pos; }
		};
	...
	///
	/// @brief sequence is an ordered container supporting constant time insertion at both end and constant removal anywhere.
	/// @tparam T value type
	...
	/// 
	/// When using the \a OptimizeForSpeed flag, the sequence will use a memory pool to allocate several buckets at once,
	...
	private:
		...
		struct Data : layout_manager
		{
			...
			Data(const Allocator& al = Allocator()) noexcept
				:layout_manager(al), size(0), dirty(NULL) {
			...
			SEQ_ALWAYS_INLINE chunk_type* endNode() noexcept { return (chunk_type*)&end; }
			SEQ_ALWAYS_INLINE const chunk_type* endNode() const noexcept { return (chunk_type*)&end; }


			void shrink_to_fit(std::vector<chunk_type*>* vec_chunk = NULL)
			{
				...
				if (size == 0 || !dirty) {
					//fill vec_chunk
					if (vec_chunk) {
						chunk_type* chunk = (chunk_type*)endNode()->next;
						while (chunk != endNode()) {
							vec_chunk->push_back(chunk);
							chunk = (chunk_type*)chunk->next;
						}
					}
					return;
				}
				...
				if (dirty != endNode()) {
					...
					size_t index = 0;
					// get allocator
					Allocator al = this->get_allocator();
					// set current node index
					node->node_index = (std::int64_t)chunks++;
					...
					try {
						...
						node->start = bit_scan_forward_64(node->used);
						node->end = bit_scan_reverse_64(node->used) + 1;
						throw;
					}
					...
					chunk_type* _del = (chunk_type*)node->next;
					if (node->start == node->end) {
						_del = node;
						last = (chunk_type*)node->prev;
					}
					...
					while (_del != endNode()) {
						...
						// Next node
						chunk_type* next = (chunk_type*)_del->next;
				...
				chunk_type* last = (chunk_type*)endNode()->prev;
				if (last->used == full)
					endNode()->prev_free = endNode()->next_free = endNode();
				else
			...
			const_iterator iterator_at(size_t pos)const  noexcept
			{
				if (pos < size / 2)
					return const_iterator((chunk_type*)end.next, end.next->start) + pos;
				else
					return const_iterator((chunk_type*)&end, 0) - (size - pos);
			}
			// Returns an iterator at given position
			iterator iterator_at(size_t pos)  noexcept
			{
				if (pos < size / 2)
					return iterator((chunk_type*)end.next, end.next->start) + pos;
				else
					return iterator((chunk_type*)&end, 0) - (size - pos);
			}
		
			// Returns distance between 2 iterators, or 0 for non random access iterators
			template<class Iter, class Cat>
			difference_type iter_distance(const Iter& it1, const Iter& it2, Cat) const noexcept { return 0; }
			template<class Iter>
			difference_type iter_distance(const Iter& it1, const Iter& it2, std::random_access_iterator_tag) const noexcept { return it1 - it2; }
			template<class Iter>
			difference_type distance(const Iter& it1, const Iter& it2)const noexcept {
		...
		void markDirty(chunk_type* start_node) noexcept {
		...
		chunk_type* make_chunk(chunk_type* prev, chunk_type* next, std::int64_t index = chunk_type::no_index)
		{
			// Allocate, might throw
			chunk_type* ptr = (chunk_type*)d_data->allocate_chunk();
			...
			if (index == chunk_type::no_index) {
				...
				if (prev == &d_data->end) {
					ptr->node_index = (next == &d_data->end) ? 0 : next->node_index - 1ULL;
				}
				else if (next == &d_data->end) {
					ptr->node_index = (prev == &d_data->end) ? 0 : prev->node_index + 1ULL;
				}
			}
			return ptr;
		}
		...
		void destroy_node_elements(chunk_type* node)
		{
			...
			if (!std::is_trivially_destructible<T>::value && node->used) {
				...
				for (int i = node->start; i < node->end; ++i)
					if(node->used & (1ULL << (std::uint64_t)i))
		...
		SEQ_ALWAYS_INLINE const T* back_ptr() const noexcept { return &(((chunk_type*)d_data->end.prev)->back()); }
		// Returns pointer to front value
		SEQ_ALWAYS_INLINE const T* front_ptr() const noexcept { return &(((chunk_type*)d_data->end.next)->front()); }
		...
		SEQ_NOINLINE(T&) emplace_back_new_chunk(chunk_type* last, Args&&... args)
		{
			...
			last = make_chunk(last, (chunk_type*)&d_data->end); //specify chunk index if not dirty
			try {
		...
		SEQ_NOINLINE(T&) emplace_front_new_chunk(chunk_type* first, Args&&... args)
		{
			...
			first = make_chunk((chunk_type*) &d_data->end, first);
			first->end = count;
		...
		void import(const sequence<T,Alloc,L,Align> & other)
		{
			...
			if (count > size()) {
				...
				if (size()) {
					...
					last = (chunk_type*)d_data->endNode()->prev;
				...
				while (chunks--) {
					last = make_chunk(last, (chunk_type*)&d_data->end);
					remove_free_node(last);
					last->used = full;
				...
				chunk_type* last = (chunk_type*)d_data->end.prev;
				difference_type diff = size() - count;

				// empty last chunk
				while (last == d_data->end.prev && diff--)
					pop_back();

				while (diff > (difference_type)chunk_type::count) {
					// Destroy full chunks
					last = (chunk_type*)d_data->end.prev;
		...
		void pop_front_remove_chunk(chunk_type* node) noexcept
		{
			...
			if (d_data->dirty == node) {
				if (node->next != d_data->endNode())
					d_data->dirty = (chunk_type*)node->next;
				else
					d_data->dirty = NULL;
			}
		...
		void pop_back_remove_chunk(chunk_type* node) noexcept
		{
			...
			if (d_data->dirty == node) {
				if (node->prev != d_data->endNode())
					d_data->dirty = (chunk_type*)node->prev;
				else
					d_data->dirty = NULL;
			}
			//remove from list
		...
		void erase_remove_chunk(chunk_type* node) noexcept
		{
			...
			if (node == d_data->dirty)
				...
				markDirty((chunk_type*)node->prev);
			else if (node->next != &d_data->end)
				markDirty((chunk_type*)node->next);
		...
		Data* make_data(const Allocator& al = Allocator())
		{
	...
	public:
		...
		/// @param al allocator object
		explicit  sequence(size_type count, const T& value, const Allocator& al = Allocator())
			:d_data(make_data(al))
		{
			resize(count, value);
		}
		...
		sequence(sequence&& other, const Allocator& alloc) noexcept(std::is_nothrow_move_assignable<T>::value)
		...
		Data* data() noexcept  { return d_data; }
		const Data* data() const noexcept { return d_data; }
		...
		std::size_t memory_footprint() const noexcept {
			return sizeof(*d_data) + d_data->memory_footprint() ;
		}

		/// @brief Returns the allocator associated with the container.
		Allocator& get_allocator() noexcept{return d_data->get_allocator();}
		/// @brief Returns the allocator associated with the container.
		Allocator get_allocator() const noexcept{return d_data->get_allocator();}

		/// @brief Returns the sequence maximum size.
		static size_type max_size() noexcept { return  LLONG_MAX ; }

		/// @brief Returns thenumber of elements in this sequence.
		size_type size() const noexcept { return d_data->size; }

		bool empty() const noexcept {return d_data->size== 0;}

		/// @brief Returns the number of elements that the container has currently allocated space for.
		size_t capacity() const noexcept { return d_data->get_capacity() * 64ULL; }

		/// @brief Returns the back sequence value.
		const T& back() const noexcept { return ((chunk_type*)d_data->end.prev)->back(); }

		/// @brief Returns the back sequence value.
		T& back() noexcept { return ((chunk_type*)d_data->end.prev)->back(); }

		/// @brief Returns the front sequence value.
		const T& front() const noexcept { return ((chunk_type*)d_data->end.next)->front(); }

		/// @brief Returns the front sequence value.
		T& front() noexcept { return ((chunk_type*)d_data->end.next)->front(); }
		...
		void clear() noexcept
		{
			...
			{
				int no_pool_count = 0;
				chunk_type* node = (chunk_type*)d_data->endNode()->next;
				while (node != d_data->endNode()) {
					// Deallocate chunk
					chunk_type* next = (chunk_type*)node->next;
					if (node->user_flag == 0)
						d_data->deallocate_chunk(node);
					node = next;
					if (++no_pool_count == 4)
			...
			// Reset members
			d_data->end.next = d_data->endNode();
			d_data->end.prev = d_data->endNode();
			d_data->endNode()->prev_free = d_data->endNode()->next_free = d_data->endNode();
			d_data->size = 0;
			d_data->dirty = NULL;
		}
		...
		/// Strong exception guarantee.
		template <class... Args>
		SEQ_ALWAYS_INLINE iterator emplace_back_iter(Args&&... args) {
			T* back = &emplace_back(std::forward<Args>(args)...);
			return iterator((chunk_type*)d_data->end.prev, (int)( back - ((chunk_type*)d_data->end.prev)->buffer()));
		}
		...
		{
			chunk_type* first = (chunk_type*)(d_data->end.next);
		...
		/// Strong exception guarantee.
		template <class... Args>
		iterator emplace_front_iter(Args&&... args) {
			T* front = &emplace_front(std::forward<Args>(args)...);
			return iterator((chunk_type*)d_data->end.next, (int)(front - ((chunk_type*)d_data->end.next)->buffer()));
		}
		...
		iterator emplace(Args&&... args)
		{
			...
			chunk_type* node = (chunk_type*)( d_data->end.next_free);
			std::uint64_t index = node->start != 0 ? node->start-1 : (node->end != count ? node->end : node->firstFree());
			...
			// Update boundaries
			if ((int)index == node->end) node->end++;
			else if ((int)index < node->start) node->start = (int)index;

			// No need to mark dirty as we insert a new object
			//markDirty(node);
		
			++d_data->size;
			return iterator(node, (int)(res - node->buffer()));
		}
		...
		void resize_front(size_type count)
		{
			...
			if (count > size()) {
				...
				if (size()) {
					...
					front = (chunk_type*)d_data->endNode()->next;
					if (front->start != 0) {
				...
				while (chunks--) {
					// Might throw, ok
					front = make_chunk((chunk_type*)&d_data->end, front);
					remove_free_node(front);
					front->used = full;
				...
				chunk_type* front = (chunk_type*)d_data->end.next;
				difference_type diff = size() - count;
				...
				while (diff > (difference_type)chunk_type::count) {
					// destroy full chunks
					front = (chunk_type*)d_data->end.next;
		...
		/// @brief Replaces the contents with count copies of value \a value
		/// @param count the new size of the container
		/// @param value the value to initialize elements of the container with
		/// Basic exception guarantee. 
		void assign(size_type count, const T& value)
		{
			assign_cat(cvalue_iterator<T>(0, value), cvalue_iterator<T>(count, value), std::random_access_iterator_tag());
		}
		...
		void pop_front() noexcept
		{
			...
			T* ptr = (T*)front_ptr();
			chunk_type* node = (chunk_type*)d_data->end.next;

			if (SEQ_UNLIKELY(node->used == full))
				add_free_node(node);

			node->used &= ~(1ULL << (std::uint64_t)node->start);
			destroy_ptr(ptr);
			if (SEQ_UNLIKELY(node->used == 0ULL)) 
				pop_front_remove_chunk(node);
			else {
				++node->start;
				if (!(node->used & (1ULL << (std::uint64_t)node->start)))
					node->start = bit_scan_forward_64(node->used);
			}
			--d_data->size;
		}
		...
		void pop_back() noexcept
		{
			...
			T* ptr = (T*)back_ptr();
			chunk_type* node = (chunk_type*)d_data->end.prev;

			if (SEQ_UNLIKELY(node->used == full))
				add_free_node(node);

			node->used &= ~(1ULL << (std::uint64_t)(ptr - node->buffer()));
			destroy_ptr(ptr);
			if (SEQ_UNLIKELY(node->used == 0ULL)) 
				pop_back_remove_chunk(node);
			else {
				if(!(node->used & (1ULL << (std::uint64_t)(--node->end -1))))
					node->end = bit_scan_reverse_64(node->used) + 1;
			}
			--d_data->size;
		}
		...
		iterator iterator_at(size_type pos) noexcept
		{
			return d_data->iterator_at(pos);
		}
		...
		/// This function is faster than using begin()+pos as it might start from the end to reach
		/// the required position.
		const_iterator iterator_at(size_type pos) const noexcept
		{
			return d_data->iterator_at(pos);
		}
		...
		iterator erase(const_iterator it) noexcept
		{
			...
			destroy_ptr(ptr);
			if (it.node->used == full)
				add_free_node(it.node);

			it.node->used &= ~(1ULL << (it.pos));
			if (SEQ_UNLIKELY(it.node->used == 0ULL)) {
				erase_remove_chunk(it.node);
			}
			else {
				markDirty(it.node);
				if(it.pos == it.node->start)
					it.node->start = bit_scan_forward_64(it.node->used);
				if(it.pos == it.node->end-1)
					it.node->end = bit_scan_reverse_64(it.node->used) + 1;
			}
			--d_data->size;
			return res;
		}
		...
		iterator erase(const_iterator first, const_iterator last) noexcept
		{
			...
			while (first != last) {
				destroy_ptr(&(*first));
				first.node->used &= ~(1ULL << (std::uint64_t)first.pos);
				++first;
				--d_data->size;
				if (node != first.node) {
		...
		void sort(Less less)
		{
			...
			iter begin(&d, (chunk_type*)(d_data->endNode()->next));
			iter end(&d, (chunk_type*)d_data->endNode(), d_data->endNode()->start, size());
		...
		iterator begin() noexcept { return iterator((chunk_type*)d_data->end.next); }
		/// @brief Returns an iterator to the element following the last element of the sequence.
		iterator end() noexcept { return iterator((chunk_type*)&d_data->end, 0); }
		/// @brief Returns an iterator to the first element of the sequence.
		const_iterator begin() const noexcept { return const_iterator((chunk_type*)d_data->end.next); }
		/// @brief Returns an iterator to the element following the last element of the sequence.
		const_iterator end() const noexcept { return const_iterator((chunk_type*)&d_data->end, 0); }
		/// @brief Returns an iterator to the first element of the sequence.
		const_iterator cbegin() const noexcept { return const_iterator((chunk_type*)d_data->end.next); }
		/// @brief Returns an iterator to the element following the last element of the sequence.
		const_iterator cend() const noexcept { return const_iterator((chunk_type*)&d_data->end, 0); }
