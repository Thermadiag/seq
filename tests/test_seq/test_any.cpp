...
namespace seq
{
	...
	struct FunInterface<R(As...)>
	{
		...
		template<class T>
		struct typed_type_info
		  : type_info
		  , any_typed_type_info<T, false> // disable hashing support
		{
			...
			virtual const std::type_info& target_type() const override { return typeid(T); }
		};
		...
		struct any_interface : Base
		{
			...
			const std::type_info& target_type() const { return this->empty() ? typeid(void) : this->type()->target_type(); }
		};
	};
